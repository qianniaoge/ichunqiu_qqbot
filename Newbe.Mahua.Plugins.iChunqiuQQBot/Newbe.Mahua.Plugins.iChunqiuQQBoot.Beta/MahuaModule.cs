...
using Newbe.Mahua.Plugins.iChunqiuQQBoot.Beta.MahuaEvents;
    ...
    public class MahuaModule : IMahuaModule
    {
        ...
        private class MahuaEventsModule : Module
        {
            protected override void Load(ContainerBuilder builder)
            {
                ...
                // 加载群成员私聊事件
                builder.RegisterType<PrivateMessageFromGroupReceivedMahuaEvent>().As<IPrivateMessageFromGroupReceivedMahuaEvent>();
                // 加载群
                builder.RegisterType<GroupMessageReceivedMahuaEvent>().As<IGroupMessageReceivedMahuaEvent>();
                // 自动接受加群
                builder.RegisterType<GroupJoiningInvitationReceivedMahuaEvent>().As<IGroupJoiningInvitationReceivedMahuaEvent>();
                // 自动接受好友
                builder.RegisterType<FriendAddingRequestMahuaEvent>().As<IFriendAddingRequestMahuaEvent>();
                // 群成员增多事件
                builder.RegisterType<GroupMemberIncreasedMahuaEvent>().As<IGroupMemberIncreasedMahuaEvent>();
            }
        }
    }
}
